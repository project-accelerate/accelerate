...
function proseGenerator(sourceDir, minLength) {
  ...
  const generator = new MarkovGenerator({
    input: flatMap(sourceFiles, (sourceFile) => {
      const source = fs.readFileSync(path.join(__dirname, sourceDir, sourceFile), 'utf8')
        .replace(/[()]/g, '')
...
function jsonGenerator(fileName) {
  const values = JSON.parse(fs.readFileSync(path.join(__dirname, fileName), 'utf8'), '\n')
  let i = -1
 ...
 */
function dateRangeGenerator({ maxDaysAhead, typicalStartHour, minDuration = 2, maxDuration = 8, rng = lowBias }) {
  return () => {
...
function getComponents(text, separator) {
  return text.split(separator).map(x => x.trim()).filter(x => x.length > 0)
}
